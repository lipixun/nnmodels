...
class A3CNetwork(object):
    """The a3c network
    """
    def __init__(self, name, actionNums, imageHeight=210, imageWidth=160, imageDepth=3, globalVars=None):
        ...
        with tf.variable_scope(name) as scope:
            ...
            with tf.variable_scope("train"):
                ...
                #
                # Create the update op. If `globalVars` is specified, then training will be applied to global vars
                #
    ...
    def update(self, states, actions, targetValues, session):
        """Update the network
        Returns:
            float: The loss
        """
        _, loss = session.run([self.updateop, self.loss], feed_dict={self.state: states, self.actions: actions, self.targetValues: targetValues})
        return loss
